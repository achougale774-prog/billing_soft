...
export default function ExpiryGuard({ children }: { children: React.ReactNode }) {
  ...
  // Global hidden keystroke listener
  useEffect(() => {
    if (!isMounted) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        // Check if buffer starts with the secret password
        if (keyBuffer.startsWith('abhishek3364')) {
          // Extract the number of days, default to 30 if none provided
          ...
          if (!isNaN(daysToAdd)) {
            ...
            setIsExpired(false);
            // Optional: alert can be removed if you want it 100% silent
            // alert(`Unlocked for ${daysToAdd} days`);
          }
        }
        keyBuffer = ''; // Reset buffer on Enter
